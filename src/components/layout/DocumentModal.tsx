...
export default function DocumentModal(
	...
	function modifyDocument() {
		...
		const currentDocuments = newestDocumentData.filter((e: IDocument) => e.name != name)

		const isDuplicated = currentDocuments.some((document: IDocument) => {
			const isSameName = document.name.toLowerCase() == newName.toLowerCase()
			const isSameFolder = document.folder == parentFolder
		...
		setDocuments(newestDocumentData.map((document: IDocument) => {
	...
	function modifyFolder() {
		...
		const isDuplicated = currentFolder.some((folder: string) => folder == newName)
		...
		setFolders(newestFolderData.map((folder: string) => {
			if(folder == name) {
				return newName
			}
    ...
    return (
        <Dialog
			...
			>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>{`${name == '' ? 'Create new' : 'Rename your'} ${newType}`}</DialogTitle>
					...
					<div className='flex justify-end space-x-2'>
						...
						<Button onClick={handleAddSubmit}>{name == '' ? 'Create' : 'Rename'}</Button>
					</div>
				</DialogContent>
			</Dialog>
    )
